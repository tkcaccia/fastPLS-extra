...
- Role: benchmark and publication source; not an installable R package
...
### C3. Obsolete companion package

Resolved after this audit: the incompatible `fastPLSextra` package scaffold,
PLS/IRLBA implementation, generated bindings, package tests, and compiled
artifacts were removed. The repository now contains only benchmark and
publication-generation source. Deposited IRLBA results remain identifiable as
external evidence and are not regenerated by a local companion package.
...
### H2. Resolved locally: installation documentation contradicted the build system

The audited `INSTALL` file said Linux and Windows required OpenBLAS and that
installation failed when it was absent. `README.md`, `DESCRIPTION`, and
`configure` correctly implemented OpenBLAS as optional and fell back to R's
BLAS/LAPACK unless explicitly required. `INSTALL` has now been corrected to
describe the implemented policy and the roles of `FASTPLS_USE_OPENBLAS=1`,
`FASTPLS_USE_OPENBLAS=0`, `OPENBLAS_ROOT`, and `fastPLS_blas()`.
...
- Preserve this policy consistently in future releases and continue to require
  an explicitly verified OpenBLAS build for published Linux/Windows performance
  runs, while keeping ordinary package installation portable.
...
### M2. Resolved locally: the generated reference manual was stale

The local `output/pdf` manual was version 0.99.64 while the package was 0.99.66.
It was not tracked by Git because generated `output/` artifacts are deliberately
ignored. The obsolete file has now been removed and replaced with a freshly
generated 24-page manual for version 0.99.66:

`output/pdf/fastPLS-reference-manual-0.99.66.pdf`

The PDF was rendered to images and visually inspected. No clipping or overlap
was found. LaTeX still reports overfull lines for several long output-field
names and weak cross-reference warnings around base plotting functions.

Recommended correction:

- Continue to generate the manual and vignette only from the frozen source
  archive.
...
1. Regenerate both from every frozen release; the local manual now matches
   version 0.99.66 and the 0.99.64 artifact has been removed.
...
3. Keep `fastPLS-extra` free of package or solver implementations.
