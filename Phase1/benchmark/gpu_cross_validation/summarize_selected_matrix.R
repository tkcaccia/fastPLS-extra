...
high_response <- raw$classifier == "regression" &
    raw$q >= 64L & raw$q / raw$n >= 0.2
...
raw$rsvd_seed <- 123L
keys <- c("platform", "dataset", "method", "classifier", "backend", "workload",
...
summary_rows <- lapply(split(raw, groups), function(block) {
    ...
    data.frame(
        ...
        classifier = block$classifier[[1L]],
...
pair_keys <- c("platform", "dataset", "method", "classifier", "backend", "precision",
...
cpu <- cv[cv$backend == "cpu", c("platform", "dataset", "method", "classifier",
    "precision", "ncomp", "control_profile", "oversample", "power",
    "rsvd_seed", "median_sec", "median_metric")]
names(cpu)[names(cpu) == "median_sec"] <- "cpu_cv_sec"
names(cpu)[names(cpu) == "median_metric"] <- "cpu_metric"
gpu <- cv[cv$backend != "cpu", ]
accelerator <- merge(gpu, cpu,
    by = c("platform", "dataset", "method", "classifier", "precision", "ncomp",
