...
REPOSITORY_ROOT="${SOURCE_ROOT:-$(CDPATH= cd -- "$(dirname "$0")/../.." && pwd)}"
PHASE1_ROOT="${REPOSITORY_ROOT}/Phase1"
PHASE2_ROOT="${REPOSITORY_ROOT}/Phase2"
FASTPLS_LIB="${FASTPLS_LIB:?Set FASTPLS_LIB to the evaluated package library}"
RESULTS_ROOT="${RESULTS_ROOT:?Set RESULTS_ROOT outside the Git checkout}"
TASK_ROOT="${TASK_ROOT:?Set TASK_ROOT to the prepared task directory}"
...
if [ -n "${EXPECTED_PACKAGE_VERSION}" ] && \
    ...
    echo "Expected fastPLS ${EXPECTED_PACKAGE_VERSION}, loaded ${actual_version}." >&2
    exit 1
fi
...
run_stage simpls_exact \
    ...
    Rscript "${PHASE1_ROOT}/benchmark/benchmark_simpls_exact_reference.R" \
        --root="${PHASE1_ROOT}" \
        --out="${RESULTS_ROOT}/validation/simpls_exact"

run_stage simpls_estimator_preservation \
    env FASTPLS_BENCH_LIB="${FASTPLS_LIB}" \
    Rscript "${PHASE1_ROOT}/benchmark/benchmark_simpls_estimator_preservation.R" \
        --root="${PHASE1_ROOT}" \
...
run_stage opls_kernel_estimator \
    env FASTPLS_BENCH_LIB="${FASTPLS_LIB}" \
    Rscript "${PHASE1_ROOT}/benchmark/benchmark_opls_kernel_estimator_validation.R" \
        --root="${PHASE1_ROOT}" \
        --out="${RESULTS_ROOT}/validation/opls_kernel_estimator"

run_stage opls_kernel_settings \
    env FASTPLS_BENCH_LIB="${FASTPLS_LIB}" \
    Rscript "${PHASE1_ROOT}/benchmark/benchmark_opls_kernel_setting_reliability.R" \
        --root="${PHASE1_ROOT}" \
        --out="${RESULTS_ROOT}/validation/opls_kernel_settings"

run_stage float32_cuda \
    env FASTPLS_BENCH_LIB="${FASTPLS_LIB}" \
    Rscript "${PHASE1_ROOT}/benchmark/benchmark_float32_backend_agreement.R" \
        --backend=cuda \
...
run_stage selected_backend_cuda \
    ...
    Rscript "${PHASE2_ROOT}/benchmark/metal_validation/run_matched_cuda_dataset_metal.R" \
...
run_stage cv_compiled_vs_r_loop_cuda \
    ...
    bash "${PHASE2_ROOT}/scripts/run_cv_compiled_vs_r_loop.sh"
